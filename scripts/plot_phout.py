...
def create_plots(rps: pd.DataFrame, status_codes: pd.DataFrame, timings: pd.DataFrame):
    fig = make_subplots(rows=2, shared_xaxes=True)

    position = {"row": 1, "col": 1}
    quantiles = [0.5, 0.9, 0.95, 0.99, 0.995, 1.0]
    timings_quantiles = timings.resample("1s").quantile(quantiles)
    colors = ["red", "orange", "gold", "lightgreen", "darkgreen", "grey"]
    ...
    for proto_code in status_codes.index.levels[0]:
        ...
        obj = go.Scatter(
            ...
            stackgroup="one",
            fillcolor=colors.get(proto_code, None),
            line_color=colors.get(proto_code, None),
        )
